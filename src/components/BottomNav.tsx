import { Home, Wallet, Clock, Target, User } from "lucide-react";
...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-t border-border/40 px-2 pb-safe-offset-2">
        ...
        {navItems.map(({ path, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] font-bold tracking-tight ${isActive ? "opacity-100" : "opacity-60"}`}>{label}</span>
            </button>
          );
        })}
      </div>
